...
# Сразу после первого входа задаём пароль root
passwd
# Или устанавливаем SSH-ключ. Не запускайте deploy-milkv.sh,
# пока root-доступ не защищён паролем или ключом.
...
Если устройство было намеренно перепрошито или заменено и его ключ действительно изменился:
```bash
ssh-keygen -R 192.168.1.100
ssh root@192.168.1.100
```

Не удаляйте ключ вслепую: неожиданная смена host key может указывать на MITM-атаку. Сначала проверьте устройство и сеть.
